...
with tab3:
    ...
    if df_proc is not None and models is not None:
        ...
        reg_pred  = np.clip(
            ...
            0.5 * models["lgb_reg"].predict(X_test), 0, 10,
        )

        from sklearn.metrics import f1_score, roc_auc_score, mean_absolute_error, r2_score
        col1, col2, col3, col4 = st.columns(4)
        col1.metric("F1 (priority class)",  f"{f1_score(y_cls_true, cls_pred):.3f}")
        col2.metric("ROC-AUC",              f"{roc_auc_score(y_cls_true, cls_proba):.3f}")
        col3.metric("MAE (impact score)",   f"{mean_absolute_error(y_reg_true, reg_pred):.3f}")
        col4.metric("R² (impact score)",    f"{r2_score(y_reg_true, reg_pred):.3f}")

        # Actual vs predicted scatter
        fig = px.scatter(
            x=y_reg_true, y=reg_pred,
            labels={"x": "Actual impact score", "y": "Predicted impact score"},
            title="Actual vs Predicted impact score (test set)",
        ...
        fig2 = px.histogram(errors, title="Prediction error distribution",
